...
export async function pollQuiz(
    courseId: string,
    lessonId: string,
    ...
    while (true) {
        const response = await fetch(
            `${import.meta.env.VITE_API_BASE_URL || (import.meta.env.DEV ? "http://localhost:5000" : "")}/api/courses/${courseId}/lessons/${lessonId}/quiz`,
            {
                credentials: "include",
            },
        );
...
export async function submitQuizAttempt(
    ...
    return fetchApi<QuizAttemptResult>(
