...
export function NotificationMenu() {
    ...
    return (
        <div className="relative" ref={dropdownRef}>
            ...
            {isOpen && (
                <div className="absolute right-0 top-full mt-2 w-80 p-0 bauhaus-square bauhaus-border bauhaus-shadow bg-card z-50">
                    <div className="flex items-center justify-between p-4 border-b bauhaus-border">
                        ...
                        {unreadCount > 0 && (
                            <button onClick={handleMarkAllAsRead} className="text-[10px] uppercase font-bold text-muted-foreground hover:text-foreground tracking-widest">
                                Mark all read
                            </button>
                        )}
                    </div>
                            ...
                            notifications.map((notification) => (
                                ...
                                >
                                    <div className="flex items-start justify-between gap-2">
                                        <h5 className="text-sm font-semibold leading-tight">{notification.title}</h5>
                                        {!notification.read && <div className="size-2 rounded-full bg-bauhaus-blue shrink-0 mt-1" />}
                                    </div>
